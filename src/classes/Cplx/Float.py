                        ...
                        if 'operator' not in list( self.dict_value.keys() ): 
                            ...
                            if self.error is None: 
                                ...
                                if self.all_data is not None:
                                    self.final_val, self.error = self.numeric.NUMERICAL(self.lex, self.DataBase,self.line).ANALYSE( mainString )
                                    if self.error is None:
                                        self.newValues = self.final_val[ 0 ]
                                        if type( self.newValues ) in [type(float()), type(int()), type(bool())]:
                                            if self.newValues >= 0:
                                                if type(self.master) in [type(float()), type(int()), type(bool())]:
                                                    self._return_   = round( self.master, self.newValues)   
                                                else: self.error = er.ERRORS( self.line ).ERROR59( )
                                            else: self.error = er.ERRORS( self.line ).ERROR60( )
                                        else: self.error = er.ERRORS( self.line ).ERROR3( 'master', 'an integer() or boolean()')  
                                    else: pass 
                                ...
                                if 'operator' not in list( self.dict_value.keys() ): 
                                    ...
                                    if self.error is None: 
                                        ...
                                        if self.all_data is not None:
                                            ...
                                            if self.error is None:
                                                self.newValues = self.final_val[ 0 ]
                                                if type( self.newValues ) in [type(bool()), type( int() )]:
                                                    if self.newValues >= 0:
                                                        if type(self.master) in [type(float()), type(int()), type(bool())]:
                                                            self._return_   = round( self.master, self.newValues)   
                                                        else: self.error = er.ERRORS( self.line ).ERROR59( )
