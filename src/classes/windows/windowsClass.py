...
class EXTERNAL_CLASS_WINDOWS:
    ...
    def TERMINAL( self, 
        ...
        self.store_value         = []
        self.class_starage       = []
        self.classes_before      = self.data_base[ 'classes' ].copy()
        self.names_before        = self.data_base[ 'class_names' ].copy()
        ###########################################################
        self.str_drop_down       = ''
        # dropdown index 
        self.drop                = 0
                ...
                if self.char:
                    ...
                    if self.char is not None:
                        ...
                        # printing and initializing of values "enter"
                            ...
                            if self.term == 'orion': print(self.main_input + self.bold+bm.words(string=self.s, color=bm.fg.rbg(255,  255, 255)).final(n=1))
                            ...
                            if self.string:
                                #calling the main module CLASSES
                                self.class_cancel, self.error = EC.EXTERNAL_CLASS(master=self.string, data_base = self.data_base, line=self.if_line,
                        ...
                        if self.border_x_limit is True:
                            ...
                            if self.term == 'orion':
                                ...
                                sys.stdout.write(self.main_input + bm.string().syntax_highlight(
                                    name=bm.words(string=self.s, color=bm.init.bold+bm.fg.rbg(255, 255, 255)).final(n=1)))
                            else:
                                                ...
                                                if v is not  None:
                                                    ...
                                                    try:
                                                        if self.indicator not in {65, 66}:
                                                            ...
                                                            sys.stdout.write(bm.clear.line(pos=0))
                                                            # re-writing string
                                                            sys.stdout.write(self.main_input+bm.string().syntax_highlight(name=bm.words(string=self.s, color=bm.fg.rbg(255, 255, 255)).final(n=1)))
