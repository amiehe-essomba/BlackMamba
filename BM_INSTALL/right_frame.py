...
def sel( value, frame1, frame2, root,  main_root, ico_path ):        
    if value.get() == 1: 
        next_btn = Button(frame2, text='next', state=NORMAL, width=10, relief=GROOVE, 
                command=lambda: nex_section(frame1=frame1, frame2=frame2, root=root, main_root=main_root, ico_path=ico_path))
...
def nex_section(frame1, frame2,  root, main_root, ico_path):
    frame1.destroy()
    frame2.destroy()
    ...
    action.action(frame1, frame2,  root, main_root, ico_path)
    ...
    prev_btn = Button(frame2, text='previous', state=NORMAL, width=10, relief=GROOVE, 
                    command=lambda : scrollbar_right(frame_right=root, main_root=main_root, ico_path=ico_path))
...
def scrollbar_right(frame_right, main_root, ico_path, license):
    ...
    agree = Checkbutton(sub_frame_right2, text='agree', variable=get_var, onvalue=1,offvalue=0, 
                    ...
                    frame2=sub_frame_right2, root=frame_right, main_root=main_root, ico_path=ico_path ))
    agree.place(x=0, y=0)
