    ...
    def Details(self, string_name: str):
        ...
        if self.list is not None:
            ...
            for j, name in enumerate(self.list):
                name = self.w+f"{self.asc['v']} "+name +f"{self.asc['v']}"
                sys.stdout.write(self.srt + '  '+ name + self.re+'\n')
                if j == 0: sys.stdout.write(self.srt + '  '+self.w+self.asc['vl']+self.asc['h']*(self.len+1)+self.asc['vr'] + self.re+'\n') 
                else: pass
                
            sys.stdout.write(self.srt + '  '+ f"{self.asc['v']}"+self.w+' '*(self.len+1)+f"{self.asc['v']}" + self.re+'\n')
