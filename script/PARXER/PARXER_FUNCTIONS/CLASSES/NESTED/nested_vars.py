...
class NESTED:
    ...
    def NESTED(self, idd : int = 0):
        ...
        if self.main_name not in self.DataBase['modulesImport']['fileNames']: 
            ...
            if self.mod['key'] is False:
                self.final_values, self.value_from_db, self.initialize_values, self.error = classInit.CLASS_TREATMENT( self.master, 
    ...
    def SUB_NESTED_VAR(self, idd : int = 0):
        self.error                  = None 
        self.final_values           = None
        self.initialize_values      = None
        self.value_from_db          = None
    ...
    def SUB_NESTED_CLASS(self ):
        
        self.new_master = self.master.copy()
        self.rest       = self.master.copy()
        self.names = self.master[ 'names' ]
        self.add_params = self.master[ 'add_params' ]
        self.expressions = self.master[ 'expressions' ]
        
        self.new_master['names'] = [self.names[0], self.names[1]]
        self.new_master['add_params'] = [self.add_params[0], self.add_params[1]]
        self.new_master['expressions' ] = [self.expressions[0], self.expressions[1]]
        
        self.rest['names'] = self.rest[ 'names' ][1 : ]
        self.rest['add_params'] = self.rest['add_params'][1 : ]
        self.rest['expressions' ] = [self.names[1]]+ self.expressions[2 : ]
        
        
        return self.new_master.copy(), self.rest.copy() 