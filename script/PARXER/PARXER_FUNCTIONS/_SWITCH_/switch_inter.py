        ...
        if self.loop_list:
            for  j, _string_ in enumerate( self.loop_list ):
                ...
                if _string_ :
                        ...
                        if self.error is None:
                            if self.active_tab is True:
                                ...
                                self.get_block, self.value, self.error = ES.EXTERNAL_BLOCKS(string=self.string,
                                    normal_string=self.normal_string,   data_base=self.data_base,  line=self.if_line).BLOCKS(  tabulation=self.tabulation, 
                    ...
                    self.error = Ie.ERRORS( self.line ).ERROR4()
                    break
        else:  self.error = Ie.ERRORS( self.line ).ERROR5( _str_='switch' )
        ############################################################################
