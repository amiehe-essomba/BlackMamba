...
class INTERNAL_UNLESS_STATEMENT:
    ...
    def UNLESS_STATEMENT(self, 
        ...
        self.tabulation             = tabulation
        self.history                = [ 'unless' ]
        self.color                  = bm.fg.rbg(199,120, 120 )
        self.before                 = end_else_elif.CHECK_VALUES(self.data_base).BEFORE()
                    ...
                    if self.active_tab is True:
                        ...
                        if self.error  is None:
                            if self.get_block   == 'begin:'     :
                                ...
                                self.error = if_statement.EXTERNAL_IF_STATEMENT(self.master,
                                ...
                                self.error = EXTERNAL_UNLESS_STATEMENT( self.master,
                                        self.data_base, self.line ).UNLESS_STATEMENT( self.value, self.tabulation + 1)
                                    ...
                                    if self.store_value:
                                        ...
                                        for _bool_ in self.boolean_store:
                                            if _bool_ == False:
                                                self.bool_key = False
                                                break
                                            else:  self.bool_key = True

                                        if self.bool_key is False: self.bool_value = True
                                        else: self.bool_value = False
...
class EXTERNAL_UNLESS_STATEMENT:
    ...
    def UNLESS_STATEMENT(self, bool_value: bool, tabulation:int):
        self.error                  = None
        self.string                 = ''
        self.normal_string          = ''
        self.end                    = ''
                    ...
                    if self.active_tab is True:
                        ...
                        if self.error  is None:
                            if self.get_block   == 'begin:'     :
                                ...
                                self.error = if_statement.INTERNAL_IF_STATEMENT(self.master,
                                            self.data_base, self.line).IF_STATEMENT(self.value, self.tabulation + 1)
                                ...
                                self.error = INTERNAL_UNLESS_STATEMENT( self.master,
                                        self.data_base, self.line ).UNLESS_STATEMENT( self.value, self.tabulation + 1)
