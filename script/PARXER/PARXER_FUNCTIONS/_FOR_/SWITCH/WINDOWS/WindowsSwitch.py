...
from script.PARXER.PARXER_FUNCTIONS._FOR_.SWITCH.WINDOWS    import externalSwitch as eS


class EXTERNAL_SWITCH_WINDOWS:
    def __init__(self, 
        data_base   : dict, 
        line        : int,
        term        : str 
            ...
            try:
                ...
                # printing and initializing of values
                    ...
                    if self.string:
                        ...
                        self.loop, self.switch_cancel, self.error = eS.EXTERNAL_SWITCH(master=self.string, data_base = self.data_base, line=self.if_line,
                            history=self.history, store_value=self.store_value, space=self.space, case_block=self.case_block).SWITCH(any_value=bool_value, 
                            tabulation=self.tabulation, loop=self.loop, _type_=_type_ )
                            
                        # break while loop if error is not None
                        if self.error is None:
                            if self.switch_cancel is True:  break
                            else:  pass
                        else:  break
                    else:
                        # if no string
