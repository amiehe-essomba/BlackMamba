...
class INTERNAL_BLOCKS:
    ...
    def BLOCKS(self, tabulation: int, loop : bool = False, function: any = None):
        self.tabulation     = tabulation
        self.back_end       = self.tabulation - 1
        self._return_       = None
        self.error          = None
                ...
                try:
                    if   self.normal_string[ : 2 ] == 'if'    :
                        ...
                        if loop in [True, False] :
                            self.error = EXTERNAL_BLOCKS(self.string, self.normal_string, 
                                                            self.data_base, self.line).END_BLOCK_TREATMENT()
                            if self.error is None:
                                self._return_ = 'end:'
                            else:pass
                        else: self.error = ERRORS( self.line).ERROR4() 
        ...
        try:
            ...
            if self.error is None:
                ...
                if self.key is True:
                    ...
                    self.error = ERRORS( self.line ).ERROR1( self.func )
    ...
    def FOR_BLOCK_TREATMENT(self, function: any = None):
        self.error              = None
        self._return_           = None
        self.value              = None
        self.key                = None
                                        ...
                                        if self.error is None:
                                            ...
                                            if self.error is None:
                                                if type( self.get_values[ 0 ] ) in [ type( tuple() ), type( list() ),
                                                    ...
                                                    if self.get_values[ 0 ]:
                                                        self._return_value_ = self.get_values[ 0 ]
                                                    else: self.error = ERRORS( self.line ).ERROR5( self.get_values[ 0 ] )
                                                else: self.error = ERRORS( self.line ).ERROR5( self.get_values[ 0 ] )
                                            else: pass
                                        else: pass
...
class ERRORS:
    ...
    def ERROR1(self, string: str = 'else'):
        error = '{}<< : >> {}is not defined at the {}end of the line. {}line: {}{}'.format(self.red, self.white, self.green,
    ...
    def ERROR6(self, value):
        error = ' {}line: {}{}'.format(self.white, self.yellow,self.line)
        self.error = fe.FileErrors('ValueError')+ '{}<< {} >> {}is {}EMPTY.'.format(self.cyan, value, self.white, self.yellow) + error
        return self.error+self.reset