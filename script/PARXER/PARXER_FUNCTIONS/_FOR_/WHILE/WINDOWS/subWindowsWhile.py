...
from script.PARXER.PARXER_FUNCTIONS._FOR_.WHILE.WINDOWS     import internalWhile as IWh


class INTERNAL_WHILE_WINDOWS:
    def __init__(self, 
        data_base   : dict, 
        line        : int,
        term        : str 
            ...
            try:
                ...
                # printing and initializing of values
                    ...
                    if self.string:
                        ...
                        self.loop, self.while_cancel, self.error = IWh.INTERNAL_WHILE(master=self.mainString, data_base = self.data_base, line=self.if_line,
                            history=self.history, store_value=self.store_value, space=self.space ).WHILE(bool_value=bool_value, tabulation=self.tabulation, 
                            loop=self.loop, _type_=_type_ , c=c, term=self.term)
                            
                        # break while loop if error is not None
                        if self.error is None:
                            if self.while_cancel is True:  break
                            else:  pass
                        else:  break
                    else:
                        # if no string
