...
class COMMENT_WINDOWS:
    ...
    def COMMENT(self,
        ...
        self.cmt_c          = self.bold+bm.fg.rbg(153, 153, 255)
        # reset color
        ...
        # counting space line
        self.space              = 0
        # used when save keyword is activated 
        self.locked             = False
        # storing comment lines
        self.comment_storage    = []
        # saving as < name > 
        self.comment_name       = None
        # break while loop
        self.begin_cancel       = False
        ###########################################################
                ...
                if self.char:
                    ...
                    if self.char is not None:
                        ...
                        # printing and initializing of values "enter"
                            ...
                            if self.string:
                                self.s, self.t, self.err = self.analyse.BUILD_CON(string=self.string, tabulation=self.tabulation)
                                if self.err is None:
                            ...
                            if self.term == 'orion': 
                                print(self.main_input + self.bold+bm.words(string=self.string, color=self.cmt_c).final(locked=self.end_comment, n=0))
                            ...
                            if self.string:
                                self.loop, self.begin_cancel, self.locked, self.error = cmt.COMMENT(master=self.string, data_base=self.data_base,
                        ...
                        if self.border_x_limit is True:
                            ...
                            if self.term == 'orion':
                                ...
                                sys.stdout.write(self.main_input + bm.string().syntax_highlight(
                                    name=bm.words(string=self.s, color=self.cmt_c).final(locked=True, n=0)))
                            else:
                                                ...
                                                if v is not  None:
                                                    ...
                                                    try:
                                                        if self.indicator not in {65, 66}:
                                                            ...
                                                            sys.stdout.write(bm.clear.line(pos=0))
                                                            # re-writing string
                                                            sys.stdout.write(self.main_input+bm.string().syntax_highlight(name=bm.words(string=self.s, 
                                                                            color=self.cmt_c).final(locked=True, n=0)))
