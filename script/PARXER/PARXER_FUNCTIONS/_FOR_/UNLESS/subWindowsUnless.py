...
from script.PARXER.PARXER_FUNCTIONS._FOR_.UNLESS            import internalUnless as iU


class INTERNAL_UNLESS_WINDOWS:
    def __init__(self, 
        data_base   : dict, 
        line        : int,
        term        : str 
        ...
        while True:
            try:
                ...
                # printing and initializing of values
                    ...
                    if self.string:
                        #calling the main module WHILE
                        self.loop, self.unless_cancel, self.error = iU.INTERNAL_UNLESS(master=self.mainString, data_base = self.data_base, line=self.if_line,
                            history=self.history, store_value=self.store_value, space=self.space, index_else=self.index_else).UNLESS(bool_value=bool_value, 
                            tabulation=self.tabulation, loop=self.loop, _type_=_type_ , c=c, term=self.term)
